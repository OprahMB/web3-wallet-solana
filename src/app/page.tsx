...
import { getProvider, signAllTransactions, signAndSendTransaction, signMessage, signTransaction, createTransferTransaction } from "./utils/function"
...
const useProps = (): Props => {
  ...
  const connection = new Connection('https://phantom.app/ul/v1/connect')
...
export default function Home() {
  ...
  return (
    <main className="w-full h-screen bg-[#ABA0F1] text-center py-20 text-white">
       <Image priority src={logo} alt="Phantom" width="200" height="100" className="mx-auto w-auto h-auto" />
      {publicKey ? (
        <div className="space-y-6">
          <h1>You connected as Solana Wallet <br /><b>{publicKey.toBase58()}</b></h1> 
          <hr className="w-2/3 mx-auto opacity-30" />
          <div className="space-y-2 w-1/2 mx-auto">
          {connectedMethods.map((method, i) => (
            <div key={`${method.name}-${i}`} onClick={method.onClick} className="cursor-pointer border border-white px-4 py-2 rounded-xl hover:opacity-50">
              {method.name}
            </div>
          ))}
          </div>
        ...
        <button onClick={handleConnect} className="cursor-pointer border border-white px-4 py-2 rounded-xl hover:opacity-50">Please Connect Wallet</button>
      }
    </main>
  );
}
